...
<div class="container">
	<div class="bn-view-common01 type01 ">
		...
		<div class="b-main-box">
			<div class="b-top-box">
				...
				<div class="b-etc-box" style="display:flex; align-items: center; gap:10px;">
					...
					<ul>
						...
						<li class="b-writer-box"><span>작성자</span> <span>${review.getMember_id()}</span>
						</li>

						<li class="b-date-box"><span>작성일</span> <span>${review.getDate() }</span>
						</li>

						<li class="b-hit-box"><span>조회수</span> <span>${ review.getView_count()}</span>
						</li>
					</ul>
				</div>
				<div class="b-etc-box">
			...
			<div class="b-content-box">
				<pre style="width: 100%; white-space: pre-wrap;">${review.getContent()}</pre>
	...
	<div class="comments-section">
		...
		<c:forEach var="reply" items="${replyList}">
			<div class="comment">
				...
				<c:if test="${ reply.getMember_id() == sessionMemberId }">
					<a
						href="reviewReplyDelete.qa?p=${param.p}&bno=${review.getBno()}&reply_id=${reply.comment_id}"
		...
		<div class="comment-input">
			<form action="reviewReply.qa" method="post">
				<!-- 게시글 ID를 서버로 보내기 위한 hidden 필드 -->
				<input type="hidden" name="bno" value="${review.bno}"> <input
	...
	<c:if test="${ review.getMember_id() == sessionMemberId || isAdmin }">
		<a href="reviewDelete.qa?p=${ param.p }&bno=${ review.getBno() }"
			class="button btn btn-success mr-sm-3 login-btn">삭제</a>
	</c:if>

	<!-- 수정 버튼: 작성자 본인인 경우에만 -->
	<c:if test="${ review.getMember_id() == sessionMemberId }">
		<a href="reviewModifyForm.qa?p=${ param.p }&bno=${ review.getBno() }"
			class="button btn btn-success mr-sm-3 login-btn">수정</a>
	</c:if>

	<!-- 글 목록 버튼 -->
	<a href="reviewList.qa?p=${ param.p }"
